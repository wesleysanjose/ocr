...
(function (DocumentViewer) {
  ...
  DocumentViewer.prototype.viewDocument = async function (caseId, documentId) {
    try {
      ...
      // Load document data with query parameters to include OCR data
      const document = await window.caseAPI.getDocument (
        caseId,
        documentId,
        true
      );
  ...
  DocumentViewer.prototype.renderDocument = function () {
    if (!this.currentDocument) {
      ...
      this.showError ('文档数据不可用');
      return;
    }

    if (!this.currentDocument.ocr_data) {
      console.error ('No OCR data available');
      this.showError ('OCR数据不可用');
      return;
    }
    ...
    if (this.elements.previewImage && pageData && pageData.preview) {
      ...
      console.error ('Preview image or data is missing');
      if (this.elements.previewImage) {
        this.elements.previewImage.alt = '预览图片不可用';
      }
    }
    ...
    if (
      pageData &&
      pageData.raw &&
      typeof this.displayOCRResults === 'function'
    ) {
  ...
  DocumentViewer.prototype.updatePageSelectors = function (totalPages) {
    ...
    if (!selectorContainer) {
      console.error ('Page selector container not found');
      return;
    }
  ...
  DocumentViewer.prototype.changePage = function (delta) {
    if (
      ...
      !this.currentDocument.ocr_data ||
      !this.currentDocument.ocr_data.pages
    ) {
      console.error ('Cannot change page: document or pages data is missing');
      return;
    }
  ...
  DocumentViewer.prototype.showOcrPage = function (page) {
    if (
      ...
      console.error ('Cannot show OCR page: document or pages data is missing');
      return;
    }
